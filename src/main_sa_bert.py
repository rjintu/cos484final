...
def main():
    ...
    parser.add_argument('--model', default='bert', type=str, required=True, help='Model type.')
    parser.add_argument('--dim', default=None, type=int, required=True, help='Dimension')
    ...
    filename = 'sa_{}_gpt2'.format(args.data)

    device = torch.device('cuda:{}'.format(args.device) if torch.cuda.is_available() else 'cpu')

    model = SAGPT().to(device)
